...
class Image(object):
    ...
    def setWidth(self,value):
        ...
        assert value > 0, repr(value) + ' is not greater than 0'
        assert len(self._data) % value == 0, repr(value) + ' does not evenly divide the # of pixels'
        self._width = value
    ...
    def __init__(self, data, width):
        ...
        """
        assert _is_pixel_list(data) == True, repr(data) + ' is not a pixel list'
        assert type(width) == int, repr(width) + ' is not an integer'
        assert width > 0, repr(width) + ' is not greater than 0'
        assert len(data) % width == 0, repr(width) + ' does not evenly divide the # of pixels'
        self._data = data
        self.setWidth(width)
    ...
    def __getitem__(self, pos):
        ...
        assert pos >= 0 and pos <= len(self._data), repr(pos) + ' is not a valid position'
    ...
    def __setitem__(self, pos, pixel):
        ...
        assert type(pos) == int, repr(pos) + ' is not an int'
        assert pos >= 0 and pos <= len(self._data), repr(pos) + ' is not a valid position'
        assert _is_pixel(pixel) == True, repr(pixel) + ' is not a valid pixel value'
    ...
    def setPixel(self, row, col, pixel):
        ...
        assert type(col) == int, repr(col) + ' is not an int'
        assert col >= 0, repr(col) + ' is not greater than or equal to 0'
        assert col < self._width, repr(row) + ' is not less than width'
        assert _is_pixel(pixel) == True, repr(pixel) + ' is not a valid pixel value'
        a = (self._width*row) + col
        self._data[a] = pixel
    ...
    def __str__(self):
        ...
        for x in range(len(self._data)):
            ...
            if x == len(self._data) - 1:
                result = result + str(self._data[x])
            #last pixel in a row
            elif x == (self._width + a) - 1:
                result = result + str(self._data[x]) + '],\n['
                a = a+self._width
            #not the last pixel in a row
            else:
                result = result + str(self._data[x]) + ', '
        return '[[' + result + ']]'
    ...
    def copy(self):
        ...
        """
        a = Image(self.getData(),self._width)
        return a
